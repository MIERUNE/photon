...
public class PhotonQueryBuilder {
    ...
    .must(QueryBuilders.matchQuery("osm_key_text", query).boost(100).analyzer("synonym_analyzer"))
    .must(QueryBuilders.matchQuery("osm_value_text", query).boost(200).analyzer("synonym_analyzer"));
        ...
        query4QueryBuilder
                ...
                .should(QueryBuilders.matchQuery(String.format("collector.%s.raw", language), query).boost(100)
                        .analyzer("search_raw"))
                .should(synonymsQuery).boost(300);
